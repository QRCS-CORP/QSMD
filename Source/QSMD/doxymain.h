...
#ifndef QSMD_DOXYMAIN_H
#define QSMD_DOXYMAIN_H

/**
 * \mainpage Quantum Secure Messaging Protocol (QSMD)
 *
 ...
 *
 * \section qsmd_sec About QSMD
 *
 * QSMD is a complete specification that not only defines a robust key exchange function but also integrates
 * authentication mechanisms and an encrypted tunnel within a single protocol. Rather than retrofitting existing
 * schemes with quantum-strength primitives, QSMD breaks new ground by introducing an entirely new set of mechanisms
 * designed from the ground up for security and performance in the post-quantum era.
 *
 * \section design_sec Design Philosophy
 *
 * Recognizing that a large-scale migration to post-quantum cryptography is inevitable, QSMD was developed without
 ...
 *
 * QSMD employs state-of-the-art cryptographic algorithms to ensure high levels of security:
 *
 * - **Asymmetric Ciphers:** QSMD supports the Kyber and McEliece asymmetric ciphers with the full range of parameter sets.
 * - **Signature Schemes:** QSMD can use Dilithium or Sphincs+ signature schemes, which were both standardized by NIST.
 * - **Symmetric Cipher:** For symmetric encryption, QSMD uses the authenticated symmetric stream cipher RCS,
 ...
 *
 * QSMD defines two complete protocol specifications that cater to different trust and performance requirements:
 *
 ...
 *
 * QSMD offers a modern, flexible, and secure alternative to traditional key exchange protocols that are now being
 * retrofitted with quantum-safe algorithms. Designed specifically for the post-quantum era, QSMD integrates robust
