...
static void logger_default_path(char* path, size_t pathlen)
{
	...
	if (path != NULL && pathlen != 0U)
	{
		...
		qsc_stringutils_concat_strings(path, pathlen, QSMD_LOGGER_PATH);
		...
		{
			qsc_folderutils_append_delimiter(path);
			qsc_stringutils_concat_strings(path, pathlen, QSMD_LOGGER_FILE);
		}
	}
}
...
{
	char buf[QSMD_LOGGING_MESSAGE_MAX + 1U] = { 0 };
	size_t lctr;
	size_t mlen;

	lctr = 0U;

	if (qsmd_logger_exists() == true)
	{
		do
		{
...
void qsmd_logger_reset(void)
{
	...
	char msg[QSMD_LOGGING_MESSAGE_MAX] = "Created: ";
	size_t mlen;
	...
	qsc_fileutils_write_line(m_log_path, QSMD_LOGGER_HEAD, sizeof(QSMD_LOGGER_HEAD) - 1U);
...
bool qsmd_logger_write(const char* message)
{
	...
	char buf[QSMD_LOGGING_MESSAGE_MAX + QSC_TIMESTAMP_STRING_SIZE + 4U] = { 0 };
	char dlm[4U] = " : ";
	size_t blen;
	size_t mlen;
	bool res;
	...
	if (message != NULL)
	{
		res = qsmd_logger_exists();
		mlen = qsc_stringutils_string_size(message);

		if (res == true && mlen <= QSMD_LOGGING_MESSAGE_MAX && mlen > 0U)
		{
